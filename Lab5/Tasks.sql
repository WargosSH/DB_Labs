...
/*-------------------Task 2.1-----------------*/
CREATE TABLE task5.publishers (
    id INT NOT NULL AUTO_INCREMENT,
    publ_house VARCHAR(256) NOT NULL,
    PRIMARY KEY (id)
);

CREATE TABLE task5.themes (
    id INT NOT NULL AUTO_INCREMENT,
    theme VARCHAR(256) NOT NULL,
    PRIMARY KEY (id)
);

CREATE TABLE task5.categories (
    id INT NOT NULL AUTO_INCREMENT,
    category VARCHAR(256) NOT NULL,
    PRIMARY KEY (id)
);

CREATE TABLE task5.books (
    id INT NOT NULL AUTO_INCREMENT,
    ...
    category_id INT,
    PRIMARY KEY (id),
    FOREIGN KEY (publ_houseID) REFERENCES task5.publishers(id),
    FOREIGN KEY (theme_id) REFERENCES task5.themes(id),
    FOREIGN KEY (category_id) REFERENCES task5.categories(id)
);


/*-------------------Task 2.2-----------------*/
INSERT INTO task5.publishers (publ_house)
SELECT DISTINCT publ_house
FROM task2.books;

INSERT INTO task5.themes (theme)
SELECT DISTINCT theme
FROM task2.books;

INSERT INTO task5.categories (category)
SELECT DISTINCT category
FROM task2.books;

INSERT INTO task5.books(book_code, is_new, name, price, publ_houseID, pages, format, book_date, book_count, theme_id, category_id)
VALUES
...
/*-------------------Task 4-----------------*/
ALTER TABLE task5.books ADD FOREIGN KEY ( publ_houseID ) REFERENCES task5.publishers (id) ON DELETE RESTRICT ON UPDATE RESTRICT ;
ALTER TABLE task5.books ADD FOREIGN KEY ( theme_id ) REFERENCES task5.themes (id) ON DELETE RESTRICT ON UPDATE RESTRICT ;
ALTER TABLE task5.books ADD FOREIGN KEY ( category_id ) REFERENCES task5.categories (id) ON DELETE RESTRICT ON UPDATE RESTRICT ;

/* У цій таблиці було створено зв'язок "1 to M", тобто один до багатьох. Інші варіанти зв'язків будуть */
/* У висновок хотілося б сказати, що такий варіант зв'язків знижує навантаженість нашої таблиці данними, та полегшує роботу з ними. */


/*-------------------Task 5-----------------*/
SELECT task5.books.name, task5.publishers.publ_house
FROM task5.books
INNER JOIN task5.publishers ON task5.books.publ_houseID = task5.publishers.id;

SELECT task5.books.name, task5.categories.category
FROM task5.books
INNER JOIN task5.categories ON task5.books.category_id = task5.categories.id;