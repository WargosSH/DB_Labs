DROP DATABASE IF EXISTS task6;
CREATE DATABASE task6;

/*-------------------Task 2.1-----------------*/
CREATE TABLE task6.publishers (
    id INT NOT NULL AUTO_INCREMENT,
    publ_house VARCHAR(256) NOT NULL,
    PRIMARY KEY (id)
);

CREATE TABLE task6.themes (
    id INT NOT NULL AUTO_INCREMENT,
    theme VARCHAR(256) NOT NULL,
    PRIMARY KEY (id)
);

CREATE TABLE task6.categories (
    id INT NOT NULL AUTO_INCREMENT,
    category VARCHAR(256) NOT NULL,
    PRIMARY KEY (id)
);

CREATE TABLE task6.books (
    id INT NOT NULL AUTO_INCREMENT,
    ...
    category_id INT,
    PRIMARY KEY (id),
    FOREIGN KEY (publ_houseID) REFERENCES task6.publishers(id),
    FOREIGN KEY (theme_id) REFERENCES task6.themes(id),
    FOREIGN KEY (category_id) REFERENCES task6.categories(id)
);


/*-------------------Task 2.2-----------------*/
INSERT INTO task6.publishers (publ_house)
SELECT DISTINCT publ_house
FROM task2.books;

INSERT INTO task6.themes (theme)
SELECT DISTINCT theme
FROM task2.books;

INSERT INTO task6.categories (category)
SELECT DISTINCT category
FROM task2.books;

INSERT INTO task6.books(book_code, is_new, name, price, publ_houseID, pages, format, book_date, book_count, theme_id, category_id)
VALUES
